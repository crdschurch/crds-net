...
class BitmovinManager {
  constructor(bitmovinConfig) {
    ...
    if (this.isStream) {
      if (this.countdown.events)
        this.streamInit(this.countdown.events, bitmovinConfig);
  ...
  scheduleFutureEvents() {
    this.events.forEach((e) => {
      const now = moment();
      const timeTilEventStart =
        moment.tz(e.start, this.dateStringFormat, this.timezoneStr) - now;
      const videoEndTime =
        moment.tz(e.start, this.dateStringFormat, this.timezoneStr) +
        this.videoDuration;
